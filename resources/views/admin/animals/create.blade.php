                ...
                <form action="{{ route('admin.animals.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('health_status') is-invalid @enderror" id="health_status" name="health_status"
                            rows="2">{{ old('health_status') }}</textarea>
                        @error('health_status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-check">
                                    <input type="checkbox" class="form-check-input" id="vaccinated" name="vaccinated"
                                        value="1" {{ old('vaccinated') ? 'checked' : '' }}>
                            ...
                            <div class="col-md-3">
                                <div class="form-check">
                                    <input type="checkbox" class="form-check-input" id="dewormed" name="dewormed"
                                        value="1" {{ old('dewormed') ? 'checked' : '' }}>
                                    <label class="form-check-label" for="dewormed">Vermifugado</label>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-check">
                                    <input type="checkbox" class="form-check-input" id="special_needs"
                                        name="special_needs" value="1" {{ old('special_needs') ? 'checked' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" id="status" name="status"
                            required>
                            <option value="disponivel" {{ old('status') == 'disponivel' ? 'selected' : '' }}>Disponível
                            </option>
                            <option value="adotado" {{ old('status') == 'adotado' ? 'selected' : '' }}>Adotado</option>
                            <option value="em_tratamento" {{ old('status') == 'em_tratamento' ? 'selected' : '' }}>Em
                                Tratamento</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
